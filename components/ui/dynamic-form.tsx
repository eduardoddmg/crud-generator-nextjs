...
export interface FormFieldConfig {
  name: string;
  label: string;
  type: "input" | "number" | "select" | "textarea" | "switch" | "checkbox" | "range";
  placeholder?: string;
  description?: string;
...
export function DynamicForm({
  ...
  const renderField = (fieldConfig: FormFieldConfig, rhfField: any) => {
    switch (fieldConfig.type) {
      ...
      case "number":
        return <Input type="number" placeholder={fieldConfig.placeholder} {...rhfField} onChange={(e) => rhfField.onChange(parseInt(e.target.value, 10))} />;
      case "textarea":
